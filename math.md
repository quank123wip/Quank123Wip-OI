...
$$
对于值域中的数值集合M\\
多项式A(x)的点值表达就是\{A(i)|i\in M\}
$$
我们可以发现，一个多项式的不同表示法的乘法也是不同的

对于系数表示法，一个$n$次多项式$A(x)$和$m$次多项式$B(x)$的积$C(x)$有
$$
C(x)=\sum_{i=0}^n\sum_{j=0}^ma_ib_j
$$
这样计算的时间复杂度是$O(nm)$的

对于点值表示法只需要对于每个取值$i$有$C(x)=A(x)\times B(x)$即可，这样计算是$O(1)$的

但显然按照常规方法进行朴素的傅里叶变换，在点值和系数之间转换，需要$O(n^2)$的时间

为了优化这一流程，我们可以利用单位根的性质，定义不多赘述，下面是单位根公式
$$
\omega_n^k=\cos\frac{2k\pi}{n}+\sin\frac{2k\pi}{n}i
$$
根据定义，显然单位根具有一个性质，即
$$
\omega_n^i=\omega_{\frac{n}{2}}^{2i}
$$
利用这样的性质，我们可以使多项式对于单位根求点值，求值过程中，对于奇偶相同的进行划分，递归/迭代进行之后整个求值的复杂度就能简化成$O(n\log{n})$
